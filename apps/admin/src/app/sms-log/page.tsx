...
export default function SmsLogPage() {
  ...
  useEffect(() => {
    adminQueries
      .getSmsOutbox(supabase)
      .then((data) => setRows(data as unknown as SmsRow[]))
  ...
  return (
    <AdminShell>
      <div className="mb-1 flex items-center justify-between">
        ...
        <Button variant="secondary" size="md" onClick={handleExportCsv} disabled={!rows || rows.length === 0}>
          Export CSV
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {rows === null ? (
          <p className="text-neutral-500">{fetchError ? "Couldn't load the SMS log." : "Loading…"}</p>
        ) : (
          <>
            {rows.map((row) => (
